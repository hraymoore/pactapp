# Deploying Pact to contrapact.net
...
## 3. Point contrapact.net at it

1. In your registrar's DNS settings, once contrapact.net is purchased:
...
2. Add `contrapact.net` (and `www.contrapact.net` if you want both) as a
...
3. Once DNS propagates, `https://contrapact.net` should reach the running
...
- Update `website/manifest.json`, `robots.txt`, `sitemap.xml`, and the
  ...
  ends up different from `contrapact.net` (a single find-and-replace).
...
- Point Stripe's webhook endpoints at
  `https://contrapact.net/api/billing/webhook` and
  `https://contrapact.net/api/identity/webhook` in the Stripe dashboard, and
